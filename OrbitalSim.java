...
public class OrbitalSim extends AbstractSimulation {
  ...
  if(time%3.1557e7 == 0&&control.getBoolean("moon?")==false){
    years++;
    if (years==1) control.println(years+" year has ellapsed.");
    else control.println(years+" years have ellapsed.");
  }
  if(time%3.155e7 == 0&&control.getBoolean("moon?")==true){
    years++;
    if (years==1) control.println(years+" year has ellapsed.");
    else control.println(years+" years have ellapsed.");
  }
      ...
      slope = ((planet.get(2).getY()-yold)/(planet.get(2).getX()-xold));
        if(slope/mold <1.000001 && slope/mold>.999999 && hyper==0){        
          ...
          y2 = planet.get(2).getY();
            ...
            //plotting the line
            for (double i=-100E11; i<50E11; i+=.5E11){
              //y = y2+m*(i-x2);
            ...
            for (double i=xmoon; i>-100E11; i-=.005E11){
  ...
  if(Double.parseDouble(ControlPanel.xposearth.getText()) > 0){
    ...
    if(control.getBoolean("Kepler's Laws") == true){
      ...
      if(passedMaxY && !graphedEllipse){ //this ensures that it doesn't keep drawing more ellipses
        ...
        centery = 0;
        B = maxY;
        A = Math.abs(Double.parseDouble(ControlPanel.xposearth.getText()) - centerx);
        ...
        if(signofx > 0){
          Circle foci1 = new Circle(Double.parseDouble(ControlPanel.xposearth.getText())-(A - C), 0, 2); //draws one foci
          Circle foci2 = new Circle(Double.parseDouble(ControlPanel.xposearth.getText()) -(A + C), 0, 2);//draws other foci
      ...
      if(signofx > 0){
        if(planet.get(1).getY() >= 0 && previousy < 0 && planet.get(1).getX() > 0 ){
          ...
          control.println("Kepler's Third Law: "+ ((periodtime*periodtime)/(A*A*A)) + " = " + (4*Math.PI*Math.PI)/(6.67E-11*Double.parseDouble(ControlPanel.masssun.getText()))); //THE LAW
        }
      }
      else if(signofx < 0){
        if(planet.get(1).getY() >= 0 && previousy < 0 && planet.get(1).getX() < 0 ){
  ...
  if(control.getBoolean("moon?")==true){
    ...
    if (control.getBoolean("Solar System")==true){
      ...
      planet.get(6-planetnum).setMass(2E30);
